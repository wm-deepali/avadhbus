...
if($_REQUEST['subview']=="add")
   {
		  ...
		  if($db->insert($data,'bk_services') == true)  
			{
			...
			if($_FILES['image']['name']!="")
				 {
				   ...
				   $exp = explode(".",$image);
				   $extension = end($exp);
				   $image_name = "services_".$date.".".$extension;
					$upload = move_uploaded_file($_FILES['image']['tmp_name'],"../gallery/".$image_name);
					if($upload)
					  {
					 $thumbnail = $obj->generateThumbnail("../gallery/".$image_name, "../gallery/thumb/".$image_name, 50,50);
						$sql2 = "UPDATE bk_services SET image_name='".$image_name."' WHERE id='".$last_id."'";
						 $update_image =	mysqli_query($db->conn,$sql2) or die(mysqli_connect_errno()."Data cannot inserted");
					  }  
				 }
			$_SESSION['services'] ="Service Added Success Fully.";
			echo "<script>window.location.href='../services.php?subview=list'</script>"; 	  
			}
		  else
			{
			$_SESSION['services'] ="Service couldn't be added.Please try again.";
			echo "<script>window.location.href='../services.php?subview=add'</script>"; 	
			}	
   }
  if($_REQUEST['subview']=="update")
   {
    ...
    $data = array("name"=>$name,
	 ...
	 $field ="id";	 
	 $id = $_REQUEST['id'];
		  if($db->update($data,'bk_services',$field,$id) == true)  
			{
			 if($_FILES['image']['name']!="")
				 {
				   ...
				   $upload = move_uploaded_file($_FILES['image']['tmp_name'],"../gallery/".$image_name);
					if($upload)
					  {
					  $thumbnail = $obj->generateThumbnail("../gallery/".$image_name, "../gallery/thumb/".$image_name, 50,50);
						$sql2 = "UPDATE bk_services SET image_name='".$image_name."' WHERE id='".$id."'";
						$update_logo =	mysqli_query($db->conn,$sql2) or die(mysqli_connect_errno()."Data cannot inserted");
					  }
				 }
			$_SESSION['services'] ="Service Update Success Fully.";
			 echo "<script>window.location.href='../services.php?subview=list'</script>"; 	 
			}
		  else
		    {
			$_SESSION['services'] ="Service couldn't be updated.Please try again.";
			 echo "<script>window.location.href='../services.php?subview=list'</script>"; 	 
			}			
   } 
  else if($_REQUEST['subview']=="bulk_actions")
   {
	   ...
	   //print_r($_POST['id']);
	    if($db->bulk_actions($table,$table_field,$action,$data) == true)
		  {
		  $_SESSION['services'] = "Action Completed Successfully.";
		  }
	   else
	      {
		  $_SESSION['services'] = "Action couldn't be Completed. Please try again.";  
		  }  
	  echo "<script>window.location.href='../services.php?subview=list'</script>"; 	  		
   } 
  else if($_REQUEST['subview']=="delete")
   {
	   ...
	   $id = $_REQUEST['id'];
	   //print_r($_POST['id']);
	    $sql2 = "SELECT * FROM bk_services WHERE id='".$id."'";
