...
if($_REQUEST['subview']=="add")
{
  ...
  { 
   ...
   if(isset($_FILES['image']['name']))
   {
    ...
    if($upload)
    {
     $thumbnail = $obj->generateThumbnail("../gallery/".$image_name, "../gallery/thumb/".$image_name,340,198);
 ...
 if(isset($_FILES['image1']['name']))
 {
  ...
  $image_name = "gallery_".$date.".".$image;
  $upload = move_uploaded_file($_FILES['image1']['tmp_name'],"../gallery/".$image_name);
  if($upload)
  {
   $thumbnail = $obj->generateThumbnail("../gallery/".$image_name, "../gallery/thumb/".$image_name,340,198);
   $sql2 = "UPDATE bk_gallery SET image_name1='".$image_name."' WHERE id='".$last_id."'";
   $update_image =  mysqli_query($db->conn,$sql2) or die(mysqli_connect_errno()."Data cannot inserted");
 } 
}
...
if($_REQUEST['subview']=="update")
{
 ...
 $field ="id";   
 $id = $_REQUEST['id'];
 if($db->update($data,'bk_gallery',$field,$id) == true)  
 {
...
{
  unlink("../gallery/".$_POST['added_image1']);
 unlink("../gallery/thumb/".$_POST['added_image1']);
 $image = $_FILES['image1']['name'];
 $exp = explode(".",$image);
 $extension = end($exp);
 $image_name = "gallery_".$date.".".$extension;
 ...
 {
  $thumbnail = $obj->generateThumbnail("../gallery/".$image_name, "../gallery/thumb/".$image_name, 340,198);
  $sql2 = "UPDATE bk_gallery SET image_name1='".$image_name."' WHERE id='".$id."'";
  $update_logo =  mysqli_query($db->conn,$sql2) or die(mysqli_connect_errno()."Data cannot inserted");
}
}
$_SESSION['gallery'] ="Gallery Update Success Fully.";
echo "<script>window.location.href='../manage_gallery.php?subview=list'</script>";    
}
else
{
  $_SESSION['gallery'] ="Gallery couldn't be updated.Please try again.";
  echo "<script>window.location.href='../manage_gallery.php?subview=list'</script>";    
} 
}
